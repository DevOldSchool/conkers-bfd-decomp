...
Use `./conker diff --record <work-item-id>` for focused verification. A nonzero
result displays the normal diff and changes no inventory. When the same command
reports `CURRENT (0)`, it updates the canonical function inventory and any
separately assigned source unit and refreshes generated progress without a
second compilation. `./conker progress match` remains a compatibility alias.
Do not edit those JSON inventories manually.
...
Exit the watcher and run `diff --record` once for authoritative evidence.
