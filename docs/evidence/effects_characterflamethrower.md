# `Effects/CharacterFlameThrower/characterflamethrower.c` US source-unit boundary
...
`../Effects/CharacterFlameThrower/characterflamethrower.c` at decompressed data
offset `0x25F28`. Code at game offset `0x197A18`, inside `func_15197A0C`,
constructs its runtime pointer. The debug beta independently retains the same
filename and anchors it inside the strongly correlated `func_1519AE4C` at debug
game offset `0x19AE58`.

The reviewed upstream US split map places the containing unit at combined
offset `0x1C2C60` and the next unit at `0x1CA420`. Subtracting the game base
`0x2D4B0` gives the US game-overlay range `0x1957B0:0x19CF70`.
