...
/* Conker US 0x17F80:0x18C60: compact-sequence decoding and loop markers.
 * The ROM retains seven members and inserts an event before track-end.
 */
enum { CONKER_TRACK_END = 19, CONKER_CSP_LOOPSTART = 20, CONKER_CSP_LOOPEND = 21 };

u32 __n_alCSeqGetTrackEvent(ALCSeq *seq, u32 track, ALEvent *event, s32 arg3);
...
void n_alCSeqNextEvent(ALCSeq *seq, ALEvent *evt, s32 arg2)
{
	u32 i;
...
u32 __n_alCSeqGetTrackEvent(ALCSeq *seq, u32 track, ALEvent *event, s32 arg3)
{
	...
	if (status == AL_MIDI_Meta) {
		...
		if (type == AL_MIDI_META_TEMPO) {
			...
			event->type = CONKER_CSP_LOOPSTART;
...
void __conker_cseq_markers(ALCSeq *seq, ALCSeqMarker *m, u32 ticks, u32 arg3)
{
	ALEvent evt;
	ALCSeq tempSeq;
	s32 i;
	...
	do {
		...
		if (evt.type == CONKER_CSP_LOOPSTART) {
