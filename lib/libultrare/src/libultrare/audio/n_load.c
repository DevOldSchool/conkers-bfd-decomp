...
/* Conker US 0x214F0:0x22040: ADPCM pull, table/reset parameters and
 * DMA chunk decoding, including the retail pointer guards and DMA failure.
 */
extern u32 __conker_audio_fault;
extern void __conker_audio_fault_handler(void);

#define ADPCMFBYTES      9
#define LFSAMPLES        4

Acmd *_decodeChunk(Acmd *ptr, ConkerNAudioPVoice *f, s32 tsam, s32 nbytes, s16 outp, s16 inp, u32 flags);

Acmd *n_alAdpcmPull(ConkerNAudioPVoice *filter, s16 *outp, s32 outCount, Acmd *p)
{
	Acmd *ptr = p;
	s16 inp;
	s32 tsam;
	...
	if (K0_TO_PHYS(f->dc_table->waveInfo.adpcmWave.book->book) > 0x800000) {
		__conker_audio_fault = 0x0f000003;
		__conker_audio_fault_handler();
	}
...
s32 n_alLoadParam(ConkerNAudioPVoice *a, s32 paramID, void *param)
{
    switch (paramID) {
    case 5:
        ...
        if (((u32)a->dc_table->waveInfo.adpcmWave.book & 0xff000003) != 0x80000000) {
...
Acmd *_decodeChunk(Acmd *ptr, ConkerNAudioPVoice *f, s32 tsam, s32 nbytes, s16 outp, s16 inp, u32 flags)
{
	...
	if (nbytes > 0) {
		...
		CONKER_AUDIO_LOAD_BUFFER(ptr++, nbytes + 8 - (nbytes & 0x7), inp, dramLoc - dramAlign);
	} else {
		dramAlign = 0;
	}
	...
	CONKER_AUDIO_ADPCM(ptr++, K0_TO_PHYS(f->dc_state), flags, tsam << 1, dramAlign, outp);
