...
After the raw base split map is available
  ...
  model-assets <survey|extract|preview|atlas|activity|compose|materials|collision|coverage|scene-consumers|verify|validate|inspect|submitted|discover-submitted> [options]
...
case "$command" in
    ...
    texture-assets)
        ...
        ;;
    model-assets)
        [[ $# -ge 1 ]] || die "usage: ./conker model-assets <survey|extract|preview|atlas|activity|compose|materials|collision|coverage|scene-consumers|verify|validate|inspect|submitted|discover-submitted> [options]"
